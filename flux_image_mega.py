...
from diffsynth.prompters import FluxPrompter
from diffsynth.models import ModelManager, load_state_dict, SD3TextEncoder1, FluxTextEncoder2, FluxDiT, FluxVAEEncoder, FluxVAEDecoder
from diffsynth.models.step1x_connector import Qwen2Connector
from diffsynth.models.flux_controlnet import FluxControlNet
from diffsynth.models.flux_ipadapter import FluxIpAdapter
from diffsynth.models.flux_value_control import MultiValueEncoder
from diffsynth.models.flux_infiniteyou import InfiniteYouImageProjector
from diffsynth.models.flux_lora_encoder import FluxLoRAEncoder, LoRALayerBlock
from diffsynth.models.tiler import FastTileWorker
from diffsynth.models.nexus_gen import NexusGenAutoregressiveModel
from diffsynth.models.nexus_gen_projector import NexusGenAdapter, NexusGenImageEmbeddingMerger
from diffsynth.utils import BasePipeline, ModelConfig, PipelineUnitRunner, PipelineUnit
from diffsynth.lora.flux_lora import FluxLoRALoader, FluxLoraPatcher, FluxLoRAFuser

from diffsynth.models.flux_dit import RMSNorm
from diffsynth.vram_management import gradient_checkpoint_forward, enable_vram_management, AutoWrappedModule, AutoWrappedLinear
...
class FluxImagePipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        self.mega_units = [
            ...
            FluxImageUnit_NoiseInitializer(),
            FluxImageUnit_PromptEmbedder(),
            MegaFluxImageUnit_InputImageEmbedder(),
            MegaFluxImageUnit_ImageIDs(),
            MegaFluxImageUnit_EmbeddedGuidanceEmbedder(),
            FluxImageUnit_LoRAEncode(),
        ]
    ...
    def training_mega_loss(self, **inputs):
        timestep_id = torch.randint(0, self.scheduler.num_train_timesteps, (1,))
        timestep = self.scheduler.timesteps[timestep_id].to(dtype=self.torch_dtype, device=self.device)
        
        inputs["latents"][1:] = self.scheduler.add_noise(inputs["input_latents"], inputs["noise"], timestep)
        training_target = self.scheduler.training_target(inputs["input_latents"], inputs["noise"], timestep)
        
        noise_pred = self.model_fn(**inputs, timestep=timestep, enable_shift_rope=True)
        
        loss = torch.nn.functional.mse_loss(noise_pred[1:].float(), training_target.float())
        loss = loss * self.scheduler.training_weight(timestep)
        return loss
    ...
    def __call__(
        ...
        if enable_shift_rope:
            for unit in self.mega_units:
                inputs_shared, inputs_posi, inputs_nega = self.unit_runner(unit, self, inputs_shared, inputs_posi, inputs_nega)
            ref_latents = inputs_shared['latents'][0:1]
        else:
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if enable_shift_rope:
                inputs_shared['latents'][0:1] = ref_latents
            # Inference
            noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep, progress_id=progress_id, enable_shift_rope=enable_shift_rope)
            if cfg_scale != 1.0:
                noise_pred_nega = self.model_fn(**models, **inputs_shared, **inputs_nega, timestep=timestep, progress_id=progress_id, enable_shift_rope=enable_shift_rope)
        ...
        image = self.vae_output_to_image(image[-1:])
...
class MegaFluxImageUnit_InputImageEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_image", "ipadapter_images", "noise", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae_encoder",)
        )

    def process(self, pipe: FluxImagePipeline, input_image, ipadapter_images, noise, tiled, tile_size, tile_stride):
        ref_image = pipe.preprocess_image(ipadapter_images).to(device=pipe.device, dtype=pipe.torch_dtype)
        ref_latents = pipe.vae_encoder(ref_image, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        if input_image is None:
        ...
        if pipe.scheduler.training:
            ...
            latents = pipe.scheduler.add_noise(input_latents, noise, timestep=pipe.scheduler.timesteps[0])
            latents = torch.cat([ref_latents, latents], dim=0)
            return {"latents": latents, "input_latents": None}
...
class MegaFluxImageUnit_ImageIDs(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("latents",))

    def process(self, pipe: FluxImagePipeline, latents):
        latent_image_ids = pipe.dit.prepare_image_ids(latents[1:])
        return {"image_ids": latent_image_ids}
...
class MegaFluxImageUnit_EmbeddedGuidanceEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("embedded_guidance", "latents"))

    def process(self, pipe: FluxImagePipeline, embedded_guidance, latents):
        guidance = torch.Tensor([embedded_guidance] * (latents.shape[0]-1)).to(device=latents.device, dtype=latents.dtype)
        return {"guidance": guidance}
...
def model_fn_flux_image(
    ...
    **kwargs
    ...
    # MegaStyle
    if enable_shift_rope:
        hidden_states = torch.cat([hidden_states[0:1], hidden_states[1:]], dim=1)
    ...
    if entity_prompt_emb is not None and entity_masks is not None:
        ...
        # MegaStyle
        if enable_shift_rope:
            ref_image_ids = image_ids.clone()
            ref_image_ids[:, :, 2] += width // 2
            image_rotary_emb = dit.pos_embedder(torch.cat((text_ids, ref_image_ids, image_ids), dim=1))
        else:
    ...
    # MegaStyle
    if enable_shift_rope:
        hidden_states = torch.cat([hidden_states[:, :sl, :], hidden_states[:, sl:, :]], dim=0)
